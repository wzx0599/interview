...
+ 指针常量和常量指针的区别？顶层`const`和底层`const`之间的区别？`this`指针是顶层`const`还是底层`const`？

  指针常量：指针的指向不可改变，指针指向的值可以改变，是顶层`const`，`int* const p = &a`

  常量指针：指针的指向可以改变，指针指向的值不可以改变，是底层`const`，`const int* p = &a`

  `this`指针是顶层`const`，即`this`指针的指向是不能发生改变的，但是其指向的内容是可以发生改变的，所以我们可以通过`swap`交换两个对象`swap(*this, *obj)`
